...
class GpuStatsLogger(Callback):
    ...
    def __init__(
        self, delay: float = 0.5, on_epoch: bool = True, on_step: bool = False
    ):
        try:
    ...
    def on_train_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        for gpu in self.monitor_epoch.average_stats.gpus:
            logged_info = {
                ...
                f"system/gpu{gpu.index}_memory_util_epoch": gpu.memory_used
                / gpu.memory_total,
                ...
                f"system/gpu{gpu.index}_power_percentage_epoch": gpu.power_draw
                / gpu.power_limit,
                f"system/gpu{gpu.index}_fan_speed_epoch": float(gpu.fan_speed)
                if gpu.fan_speed is not None
                else 0.0,
            }
            pl_module.logger.log_metrics(logged_info, step=trainer.global_step)
            pl_module.log_dict(
                logged_info, logger=False, sync_dist=pl_module.distributed
            )
    ...
    def on_train_batch_end(
        ...
        for gpu in self.monitor_step.average_stats.gpus:
            logged_info = {
                ...
                f"system/gpu{gpu.index}_memory_util_step": gpu.memory_used
                / gpu.memory_total,
                ...
                f"system/gpu{gpu.index}_power_percentage_step": gpu.power_draw
                / gpu.power_limit,
                f"system/gpu{gpu.index}_fan_speed_step": float(gpu.fan_speed)
                if gpu.fan_speed is not None
                else 0.0,
            }
            pl_module.logger.log_metrics(logged_info, step=trainer.global_step)
