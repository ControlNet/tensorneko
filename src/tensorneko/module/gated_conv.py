...
from ..layer.masked_conv2d import (
...
class GatedConv(NekoModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.Conv = {
            "vertical": _VerticalStackConv2dA
            if mask_type == "A"
            else _VerticalStackConv2dB,
            "horizontal": _HorizontalStackConv2dA
            if mask_type == "A"
            else _HorizontalStackConv2dB,
        }

        self.conv_vertical = self.Conv["vertical"](
            in_channels, out_channels, kernel_size, **kwargs
        )
        self.conv_horizontal = self.Conv["horizontal"](
            in_channels, out_channels, kernel_size, **kwargs
        )
